      ...
      <div className="w-full h-full p-2">
        {SidebarRoutes.map((item,index) => {
          return <div onClick={()=> {
              ...
              push(item.route)
            }} key={index+1} className={`w-full h-10 rounded-md my-2 flex flex-row justify-start items-center gap-4 px-5 hover:cursor-pointer ${ active == index ? 'bg-gray-800' : 'bg-transparent' } hover:bg-gray-800 transition-all`}>
            {item.icon}
            <span className={`text-sm tracking-wider ${active == index ? 'text-white font-medium' : 'text-gray-300'}`}>{item.label}</span>
          </div>
        }) }
      </div>
    </div>
